...
export class HistorialRecorridos {
    ...
    calcularMetricas(): {
        ...
        const velocidades = this.ubicaciones.map(u => u.getVelocidad()).filter(v => v > 0);
        const velocidadPromedio = velocidades.length > 0 ? velocidades.reduce((sum, v) => sum + v, 0) / velocidades.length: 0;
